...
public class SubstrateRuntimeConfigurationBuilder extends SharedRuntimeConfigurationBuilder {
    ...
    protected Replacements createReplacements(Providers p) {
        ...
        return new SubstrateReplacements(p, bytecodeProvider, SubstrateTargetDescription.singleton(), new SubstrateGraphMakerFactory());
    }
    ...
    protected SharedCodeCacheProvider createCodeCacheProvider(RegisterConfig registerConfig) {
        if (SubstrateOptions.supportCompileInIsolates()) {
            return new IsolateAwareCodeCacheProvider(SubstrateTargetDescription.singleton(), registerConfig);
        }
        return new SubstrateCodeCacheProvider(SubstrateTargetDescription.singleton(), registerConfig);
    }
}
