            ...
            switch (token.kind) {
                ...
                case wordBoundary:
                    ...
                    if (getLocalFlags().isUnicode(mode)) {
                        astBuilder.addWordBoundaryAssertion(token, lexer.getPredefinedCharClass('w'), lexer.getPredefinedCharClass('W'));
                    } else if (getLocalFlags().isLocale()) {
                        astBuilder.addWordBoundaryAssertion(token, lexer.getLocaleData().getWordCharacters(), lexer.getLocaleData().getNonWordCharacters());
                    } else {
                        astBuilder.addWordBoundaryAssertion(token, Constants.WORD_CHARS, Constants.NON_WORD_CHARS);
                    }
                    break;
