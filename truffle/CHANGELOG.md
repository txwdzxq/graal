...
* GR-73872: Added `constant` attribute to the `@Option` annotation. When set to `true` together with a field of type `ConstantOptionKey<T>`, the option's value is fixed at class initialization time from the system property `-Dpolyglot.image-build-time.<option-name>=<value>` (or the declared default if the property is absent). The GraalVM compiler constant-folds calls to `ConstantOptionKey.getConstantValue()` on `static final` fields, eliminating dead branches from compiled Truffle interpreters.
