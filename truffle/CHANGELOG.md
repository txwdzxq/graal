...
* GR-73707: Bytecode DSL: Added `BytecodeRootNode#traceTransition(BytecodeTransition, Frame)`, an optional hook invoked whenever the interpreter transitions between bytecode nodes due to deoptimization (`isTransferToInterpreter()`), a bytecode update (`isBytecodeUpdate()`), or source information materialization (`isSourceInformationUpdate()`).
...
* GR-73707: Added engine option `engine.TraceBytecodeTransition` to log bytecode interpreter transition events engine-wide. Supported values are `true` (all kinds), `transferToInterpreter`, `source`, `bytecode`, `tier`, `tag`, and `instrumentation` (comma-separated for combinations). Use `engine.BytecodeMethodFilter` and `engine.BytecodeLanguageFilter` to scope tracing.
